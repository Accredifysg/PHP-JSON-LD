...
- **Scoped contexts now inherit the default `@direction`, completing §4.1
  ...
  no movement), and PyLD shared the omission
  ...
  in final review; Ruby json-ld and Titanium JSON-LD inherit per spec. With
  this change we join the spec-conformant majority instead of staying pinned
  to a stalled upstream. **Default-mode N-Quads, canonical hashes and
